...
@app.route('/logs/<string:log_type>/<int:year>/<int:month>', methods=['GET'])
def get_logs(log_type, year, month):
    """
    retrieve log files
    """

    if log_type not in ['train', 'pred']:
        print("ERROR API (get_logs): invalid log type")
        abort(400)

    logfile = f'../logs/{log_type}-{year}-{month}.log'
